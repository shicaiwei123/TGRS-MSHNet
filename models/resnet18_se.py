...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers,input_channel, args, num_classes=1000, zero_init_residual=False,
        ...
        if args.modal == 'multi':
            self.inplanes = 256
            self.layer3_new = self._make_layer(block, 256, layers[2], stride=1,
...
def resnet18_se(input_channel,args, pretrained=False, **kwargs):
    """Constructs a ResNet-18 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(BasicBlock, [2, 2, 2, 2], input_channel=input_channel, args=args, num_classes=args.class_num, **kwargs)
    if not pretrained:
        return model
    else:
